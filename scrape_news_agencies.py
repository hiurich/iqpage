...
OUTPUT_FILE = "news_agencies_ny.csv"
...
def extract_emails_from_url(url):
    ...
    try:
        ...
        filtered = [e for e in set(emails) if not any(skip in e.lower() for skip in
                    ["example", "yourdomain", "domain.com", "email.com", "sentry", "wixpress"])]
        return "; ".join(filtered[:5])
    except Exception:
        return ""

def main():
    print("Searching Google Maps for News Agencies in New York...")
    places = search_maps("News Agency New York")
    ...
    for place in places:
        ...
        time.sleep(1)  # be polite to target servers
